<div class="flex items-center justify-center py-10 lg:px-0 sm:px-6 px-4 mt-10">
  ...
  <a href="?pagina=<?php echo ($pagina > 1) ? $pagina - 1 :  1 ?>&<?php echo "precio=$query_precio&fecha=$query_fecha&artista=".urlencode($query_artista)."&genero=".urlencode($query_genero)."&estado=$query_estado" ?>">
    ...
    <div class="sm:flex hidden">
      ...
      for ($i = 1; $i <= $total_paginas; $i++) {
        if ($pagina == $i) {
          ...
          echo "<a href='?pagina=$i&precio=$query_precio&fecha=$query_fecha&artista=".urlencode($query_artista)."&genero=".urlencode($query_genero)."&estado=$query_estado' class='text-sm font-medium leading-none cursor-pointer text-gray-600 hover:text-blue-700 border-t border-transparent hover:border-blue-400 pt-3 mr-4 px-2'>$i</a>";
        }
      }
    ...
    <a href="?pagina=<?php echo ($pagina < $total_paginas) ? $pagina + 1 : $total_paginas ?>&<?php echo "precio=$query_precio&fecha=$query_fecha&artista=".urlencode($query_artista)."&genero=".urlencode($query_genero)."&estado=$query_estado" ?>">
      <div class="flex items-center pt-3 text-gray-600 hover:text-blue-700 cursor-pointer">
